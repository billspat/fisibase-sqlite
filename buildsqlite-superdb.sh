...
SQLDIR='buildsql-superdb'

# list of tables must be in certain order, as some of 'fix' sql depends on
# previous tables be present

tables="tblSessions tblGreetings tblHyenas tblAggression_talek tblHyenasPerSession tblLandmarks tblLocationsPerSession tblPreyDensity tblRanks tblDarting_talek "
...
# tables to do: "tblPredatorsPerSession, tblEats, tblFeeding
...
# if there is a file called 'pre' then run it 
# parameterize this file name?
if [[ -e "$SQLDIR/pre.sql" ]]; then
    sqlite3 "$sqlitefile"  < "$SQLDIR/pre.sql"
fi
...
# if there is a file called 'post' then run it 
# removes tables and 
# parameterize this file name?
if [[ -e "$SQLDIR/post.sql" ]]; then
    echo "***running post.sql"
    sqlite3 "$sqlitefile"  < "$SQLDIR/post.sql"
fi
...
echo "----------"
for table in $tables; do
    rowsinserted=$(sqlite3 "$sqlitefile"  -cmd "select count(*) from ${table}" < /dev/null)
    echo $table  $rowsinserted
done
